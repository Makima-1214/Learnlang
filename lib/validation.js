...
export const schemas = {
  ...
  followSchema: (data) => {
    const errors = [];
    if (!data.followingId || typeof data.followingId !== "string")
      errors.push("Invalid followingId");
    return errors.length > 0 ? { valid: false, errors } : { valid: true };
  },

  searchFriendsSchema: (data) => {
    const errors = [];
    if (!data.query || !validateString(data.query, 1, 100))
      errors.push("Search query must be 1-100 characters");
    if (data.page && !validateInt(data.page, 1, 1000))
  ...
  blockSchema: (data) => {
    ...
    if (data.reason && !validateString(data.reason, 0, 500))
      errors.push("Reason must be 0-500 characters");
    return errors.length > 0 ? { valid: false, errors } : { valid: true };
  },
