  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(
        `/api/admin/quizzes/${deleteDialog.quiz.id}`,
        {
          method: "DELETE",
        },
      );
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          {[
            ...
            <Card
              key={i}
              className={`duo-card border-4 ${stat.border} shadow-none ${stat.bg}`}
            >
              <CardHeader className="pb-2">
                <div className="flex items-center justify-between">
                  <CardDescription className="font-black uppercase text-[10px] tracking-widest text-gray-500">
                    {stat.label}
                  </CardDescription>
                  {stat.icon}
                </div>
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="hover:bg-transparent border-b-2">
                      <TableHead className="w-24 text-center font-black text-gray-400">
                        Level
                      </TableHead>
                      <TableHead className="font-black text-gray-400">
                        Kuis & Deskripsi
                      </TableHead>
                      ...
                      <TableHead className="text-right font-black text-gray-400 pr-8">
                        Aksi
                      </TableHead>
                    </TableRow>
                  </TableHeader>
                        ...
                        >
                          <TableCell className="text-center">
                            <div
                              ...
                              style={{
                                backgroundColor: quiz.color || "#8B5CF6",
                          ...
                          <TableCell className="text-center">
                            <Badge
                              variant="outline"
                              className="font-black border-2 rounded-lg"
                            >
                              {quiz._count?.questions || 0} Pertanyaan
                            </Badge>
                          </TableCell>
                          <TableCell className="text-center">
                            <Badge className="font-black border-2 rounded-lg bg-emerald-50 text-emerald-700 border-emerald-100">
                              +{quiz.rewardXp || 0} EXP
                            </Badge>
                          </TableCell>
                          <TableCell className="text-center">
                            <Badge className="font-black border-2 rounded-lg bg-amber-50 text-amber-700 border-amber-100">
                              {quiz.minXp || 0} EXP
                            </Badge>
                          </TableCell>
                          <TableCell className="text-center">
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="font-bold text-gray-500">
              ...
              <span className="text-red-500">
                &quot;{deleteDialog.quiz?.title}&quot;
              </span>
              ? Semua data hasil kuis user juga akan ikut terhapus secara
              permanen.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className="gap-2">
            <AlertDialogCancel className="duo-btn-primary rounded-xl font-black border-2 border-b-4">
              Batal
            </AlertDialogCancel>
            <AlertDialogAction
