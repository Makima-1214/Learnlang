...
export default function TakeQuizPage() {
  ...
  const fetchQuiz = async () => {
    try {
      ...
      else { toast.error("Quiz tidak ditemukan"); router.push("/quiz"); }
  ...
  const confirmSubmit = async () => {
    ...
    try {
      ...
      if (res.ok) { setResult(await res.json()); toast.success("Quiz selesai!"); }
      else toast.error("Gagal mengirim jawaban");
    } catch { toast.error("Gagal mengirim jawaban"); }
      ...
      <DashboardLayout>
        <div className="min-h-screen bg-gradient-to-br from-[#F0F7FF] via-[#EEF2FF] to-[#F5F3FF] flex items-center justify-center">
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="font-black text-indigo-500 animate-pulse">Memuat quiz...</p>
          </div>
        </div>
      </DashboardLayout>
    );
        ...
        <div className="min-h-screen bg-gradient-to-br from-[#F0F7FF] via-[#EEF2FF] to-[#F5F3FF] font-[family-name:var(--font-nunito)] py-8 px-4">
          ...
          >
            <div className="bg-white border-2 border-b-[6px] border-gray-200 rounded-[2rem] overflow-hidden shadow-xl">
              {/* Header */}
              <div className={`px-8 pt-10 pb-8 text-center relative overflow-hidden ${isWin ? "bg-gradient-to-br from-emerald-400 via-teal-500 to-emerald-600" : "bg-gradient-to-br from-rose-400 via-red-500 to-rose-600"}`}>
                <div className="absolute inset-0 opacity-10 text-[120px] leading-none text-center select-none">{isWin ? "🏆" : "🔄"}</div>
                <div className="relative">
                  ...
                  <p className="text-white/80 text-sm font-bold">
                    {isWin ? "Kamu berhasil menyelesaikan quiz ini!" : "Jangan menyerah, terus berlatih!"}
                  </p>
                </div>
              </div>
              ...
              <div className="p-6">
                ...
                <div className="flex justify-center mb-5">
                  <div className={`w-28 h-28 rounded-full border-[6px] flex flex-col items-center justify-center shadow-inner ${isWin ? "border-emerald-400 bg-emerald-50" : "border-rose-400 bg-rose-50"}`}>
                    <span className={`text-4xl font-black leading-none ${isWin ? "text-emerald-600" : "text-rose-600"}`}>{pct}%</span>
                ...
                <div className="grid grid-cols-3 gap-3 mb-5">
                  {[
                    { icon: "solar:check-circle-bold", iconClass: "text-emerald-500", bg: "bg-emerald-50 border-emerald-200", val: `${result.score}/${result.totalQuestions}`, valClass: "text-emerald-600", label: "Benar" },
                    ...
                    { icon: "solar:book-2-bold",       iconClass: "text-amber-500",   bg: "bg-amber-50 border-amber-200",    val: result.totalQuestions,                       valClass: "text-amber-600",   label: "Soal" },
                ...
                <div className="flex gap-3">
                  <button className="duo-btn flex-1 py-3.5 bg-white text-gray-600 border-2 border-b-4 border-gray-200 rounded-2xl font-black text-sm flex items-center justify-center gap-2" onClick={() => router.push("/quiz")}>
                    <Icon icon="solar:home-2-bold" className="text-lg" /> Kembali
                  </button>
                  <button className="duo-btn flex-1 py-3.5 bg-indigo-500 text-white border-2 border-b-4 border-indigo-700 rounded-2xl font-black text-sm flex items-center justify-center gap-2"
                    onClick={() => { setResult(null); setAnswers({}); setCurrentQuestion(0); setRevealed(false); setCorrect(null); }}>
                    <Icon icon="solar:restart-bold" className="text-lg" /> Kerjakan Lagi
                  </button>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-3xl border-2 border-b-[6px] border-gray-200 shadow-sm overflow-hidden">
              <div className="flex items-center gap-2 px-5 py-4 border-b-2 border-gray-100">
                <Icon icon="solar:clipboard-list-bold" className="text-xl text-indigo-500" />
                <h3 className="font-black text-gray-900">Pembahasan Jawaban</h3>
              </div>
              <div className="p-5 space-y-3">
                {result.detailedResults.map((item, index) => (
                  <div key={index} className={`rounded-2xl border-2 border-b-4 p-4 ${item.isCorrect ? "bg-emerald-50 border-emerald-200" : "bg-red-50 border-red-200"}`}>
                    <div className="flex items-start gap-3">
                      <div className={`w-8 h-8 rounded-xl flex items-center justify-center shrink-0 mt-0.5 ${item.isCorrect ? "bg-emerald-400" : "bg-red-400"}`}>
                        <Icon icon={item.isCorrect ? "solar:check-circle-bold" : "solar:close-circle-bold"} className="text-white text-lg" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="font-black text-gray-900 text-sm mb-2">{index + 1}. {item.question}</p>
                        <div className="space-y-1 text-xs font-bold">
                          <p className="text-gray-600">Jawaban kamu: <span className={item.isCorrect ? "text-emerald-700" : "text-red-700"}>{item.userAnswer || "(tidak dijawab)"}</span></p>
                          {!item.isCorrect && <p className="text-gray-600">Jawaban benar: <span className="text-emerald-700">{item.correctAnswer}</span></p>}
                        </div>
                      </div>
                    </div>
                  </div>
      ...
      <div className="flex-1 flex flex-col min-h-screen bg-gradient-to-br from-[#F0F7FF] via-[#EEF2FF] to-[#F5F3FF] font-[family-name:var(--font-nunito)]">
        ...
        <div className="max-w-5xl w-full mx-auto px-2 sm:px-6 py-2 md:py-4 relative z-10 flex-1 flex flex-col pb-8">
          ...
          <div className="flex items-center gap-2 mb-3 bg-white/80 backdrop-blur-sm rounded-xl p-2 border-2 border-white shadow-sm">
            ...
            >
              <Icon icon="solar:arrow-left-bold" className="w-3 h-3 sm:w-3.5 sm:h-3.5" />
              <span className="hidden sm:inline">Keluar</span>
            </button>
            <div className="flex-1 flex items-center gap-1 px-2">
              {quiz.questions.map((_, i) => (
                <div key={i} className={`h-1.5 sm:h-2 flex-1 rounded-full transition-all duration-300 ${
                  i < currentQuestion ? "bg-[#10B981]" :
                  i === currentQuestion ? "bg-gradient-to-r from-[#6366F1] to-[#818CF8] animate-pulse" :
                  "bg-gray-200"
                }`} />
              ))}
            </div>
            <div className="shrink-0 px-2.5 py-1 bg-indigo-50 border-2 border-indigo-200 rounded-xl">
              <span className="font-black text-indigo-600 text-xs">{answeredCount}/{total}</span>
            </div>
          </div>
          ...
          <div className="grid lg:grid-cols-12 gap-3 sm:gap-6 items-start flex-1 pb-1">
            ...
            <div className="hidden lg:flex lg:col-span-4 flex-col items-center justify-start gap-4 bg-white/50 border-2 border-white/80 rounded-3xl p-6 shadow-sm sticky top-4">
              ...
              <div className="w-full text-center space-y-2">
                <p className="font-black text-gray-800 text-sm leading-snug">{quiz.title}</p>
                {quiz.difficulty && (
                  <span className={`inline-block px-3 py-1 font-black text-[11px] rounded-xl border-2 ${
                    quiz.difficulty === "EASY"   ? "bg-emerald-50 border-emerald-200 text-emerald-700" :
                    quiz.difficulty === "MEDIUM" ? "bg-amber-50 border-amber-200 text-amber-700" :
                ...
                {revealed && (
                  ...
                  >
                    <div className="flex items-center justify-center gap-2 mb-1">
                      <div className="w-7 h-7 rounded-full flex items-center justify-center text-white font-black text-sm" style={{ backgroundColor: correct ? "#10B981" : "#EA2B2B" }}>
                        {correct ? "✓" : "✗"}
                      </div>
                      <span className="font-black text-sm" style={{ color: correct ? "#059669" : "#C62828" }}>
                        {correct ? "Jawaban Tepat!" : "Bukan yang itu!"}
                      </span>
                    </div>
                    <p className="text-xs font-bold leading-relaxed" style={{ color: correct ? "#047857" : "#E53935" }}>
                      {correct ? "Kamu benar! 👍" : `Jawaban benar: ${question?.options.find(o => o.isCorrect)?.option}`}
                    </p>
                  </motion.div>
                )}
              </AnimatePresence>
              ...
              <div className="w-full">
                <p className="text-[10px] font-black text-gray-400 uppercase tracking-wider mb-2">Navigasi Soal</p>
                <div className="flex flex-wrap gap-1.5">
                  {quiz.questions.map((q, i) => (
                    <button key={i} onClick={() => setCurrentQuestion(i)}
                      className={`w-8 h-8 rounded-xl font-black text-xs border-2 border-b-[3px] transition-all ${
                        i === currentQuestion ? "bg-indigo-500 border-indigo-700 text-white scale-110" :
                        answers[q.id] ? "bg-emerald-100 border-emerald-300 text-emerald-700" :
            ...
            <div className="col-span-12 lg:col-span-8 flex flex-col">
              <div className="bg-white border-2 border-b-[4px] sm:border-b-[6px] border-gray-200 rounded-2xl sm:rounded-3xl p-3 sm:p-5 shadow-sm flex flex-col gap-3">
                ...
                <div className="hidden lg:flex bg-[#E0E7FF] border-2 border-[#818CF8] rounded-2xl p-3 items-center gap-3">
                  <Icon icon="solar:question-circle-bold" className="text-xl text-indigo-500 shrink-0" />
                  <div>
                    <h4 className="font-black text-[#4338CA] text-[10px] uppercase tracking-wider">Quiz:</h4>
                    <p className="text-[#3730A3] font-black text-sm leading-tight mt-0.5">Pilih jawaban yang paling tepat!</p>
                  </div>
                </div>
                ...
                {revealed && (
                  <div className="block lg:hidden">
                    ...
                    >
                      ...
                      <p className="text-[11px] font-bold mt-0.5" style={{ color: correct ? "#047857" : "#E53935" }}>
                        {correct ? "Kamu benar! 👍" : `Jawaban benar: ${question?.options.find(o => o.isCorrect)?.option}`}
                      </p>
                    </motion.div>
                  </div>
                ...
                <AnimatePresence mode="wait">
                  ...
                  >
                    <div className="rounded-3xl bg-white/90 border border-gray-200 p-5 sm:p-6 text-center shadow-sm relative overflow-hidden">
                      ...
                      <p className="text-lg sm:text-xl font-black text-[#1E1B4B] leading-snug">{question.question}</p>
                    </div>
                    ...
                    <div className="grid gap-2.5 sm:grid-cols-2 mt-3">
                      {question.options.map((option, idx) => {
                        ...
                        const isCorrectOpt = revealed && option.id === correctOptionId;
                        return (
                          <button
                            key={option.id}
                            type="button"
                          ...
                          >
                            <span className={`w-7 h-7 rounded-xl flex items-center justify-center font-black text-xs shrink-0 transition-colors ${
                              ...
                              isCorrectOpt      ? "bg-emerald-300 text-white" :
                              "bg-gray-100 text-gray-300"
                            }`}>{LABELS[idx] ?? idx + 1}</span>
                            <span className="flex-1 leading-snug">{option.option}</span>
                            {isSel && correct  && <span className="text-emerald-500 text-base">✓</span>}
                            {isSel && !correct && <span className="text-red-400 text-base">✗</span>}
                            {isCorrectOpt && !isSel && <span className="text-emerald-400 text-xs font-black">✓</span>}
                          </button>
                        );
                      })}
                    </div>
                ...
                <div className="pt-3 border-t border-gray-100 flex items-center gap-3">
                  ...
                  {currentQuestion === total - 1 ? (
                    ...
                    >
                      ...
                      {revealed && <Icon icon="solar:arrow-right-bold" className="text-base" />}
                    </motion.button>
                  )}
                </div>
              </div>
        ...
        <AlertDialog open={exitDialogOpen} onOpenChange={setExitDialogOpen}>
          <AlertDialogContent className="rounded-3xl border-4 border-b-[6px] border-gray-200 font-[family-name:var(--font-nunito)]">
            <AlertDialogHeader>
              <AlertDialogTitle className="font-black">Keluar dari Quiz?</AlertDialogTitle>
              <AlertDialogDescription className="font-bold text-gray-500">Progress kamu akan hilang dan tidak tersimpan.</AlertDialogDescription>
            </AlertDialogHeader>
            <div className="flex gap-3 justify-end">
              <AlertDialogCancel className="rounded-2xl font-black border-2 border-b-4">Lanjutkan</AlertDialogCancel>
              <AlertDialogAction onClick={() => router.push("/quiz")} className="rounded-2xl font-black bg-red-500 border-2 border-b-4 border-red-700 hover:bg-red-600">Keluar</AlertDialogAction>
            </div>
          </AlertDialogContent>
        </AlertDialog>
        ...
        <AlertDialog open={submitDialogOpen} onOpenChange={setSubmitDialogOpen}>
          <AlertDialogContent className="rounded-3xl border-4 border-b-[6px] border-gray-200 font-[family-name:var(--font-nunito)]">
            <AlertDialogHeader>
              <AlertDialogTitle className="font-black">Kirim Jawaban?</AlertDialogTitle>
              <AlertDialogDescription className="font-bold text-gray-500">Kamu tidak bisa mengubah jawaban setelah ini.</AlertDialogDescription>
            </AlertDialogHeader>
            <div className="flex gap-3 justify-end">
              <AlertDialogCancel className="rounded-2xl font-black border-2 border-b-4">Kembali</AlertDialogCancel>
              <AlertDialogAction onClick={confirmSubmit} disabled={isSubmitting} className="rounded-2xl font-black bg-emerald-500 border-2 border-b-4 border-emerald-700 hover:bg-emerald-600">
